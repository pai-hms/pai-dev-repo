...
class ChatSessionRepository:
    ...
    def find_session_by_id(self, thread_id: str) -> Optional[ChatSession]:
        """ID로 세션 조회"""
    ...
    def find_active_sessions(self) -> Dict[str, ChatSession]:
        """활성 세션만 조회"""
        return {
            tid: session for tid, session in self._sessions.items() 
    ...
    def save_message(self, message: ChatMessage) -> None:
        ...
        # 세션의 메시지 카운트 업데이트
        if message.thread_id in self._sessions:
            self._sessions[message.thread_id].increment_message_count()
    
    def find_messages_by_thread(self, thread_id: str) -> List[ChatMessage]:
        """스레드별 메시지 조회"""
        return self._messages.get(thread_id, []).copy()  # 불변성 보장
